...
typedef BOOL(WINAPI* _GetMailslotInfo) (HANDLE hMailslot, LPDWORD lpMaxMessageSize,LPDWORD lpNextSize, LPDWORD lpMessageCount, LPDWORD lpReadTimeout);
typedef HANDLE(WINAPI* _CreateMailslotA) (LPCSTR lpName, DWORD nMaxMessageSize, DWORD lReadTimeout, LPSECURITY_ATTRIBUTES lpSecurityAttributes);
typedef HANDLE(WINAPI* _CreateFileA) (LPCSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition, DWORD wFlagsAndAttributes, HANDLE hTemplateFile);
/*BOF Defs*/
...
#define FREE( adr ) KERNEL32$HeapFree(KERNEL32$GetProcessHeap(), 0, adr)
...
WINBASEAPI BOOL WINAPI KERNEL32$HeapFree(HANDLE hHeap, DWORD dwFlags, _Frees_ptr_opt_ LPVOID dwBytes);
...
//API for breakpoint
WINBASEAPI VOID WINAPI KERNEL32$InitializeCriticalSection(LPCRITICAL_SECTION lpCriticalSection);
WINBASEAPI VOID WINAPI KERNEL32$EnterCriticalSection(LPCRITICAL_SECTION lpCriticalSection);
WINBASEAPI VOID WINAPI KERNEL32$LeaveCriticalSection(LPCRITICAL_SECTION lpCriticalSection);
WINBASEAPI VOID WINAPI KERNEL32$DeleteCriticalSection(LPCRITICAL_SECTION lpCriticalSection);
